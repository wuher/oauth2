...
// 
// This is a shameless port of Joe Stump's / SimpleGeo's python-oauth2
// library at http://github.com/simplegeo/python-oauth2/
...
Request.prototype.toUrl = function () {
    ...
    for (k in this.parameters) if (this.parameters.hasOwnProperty(k)) {
        if (sz.length !== 0) {
            sz += "&";
        }
...
Request.makeTimestamp = function () {
    return util.string(Math.round(new Date().getTime() / 1000));
};
...
Client.prototype.request = function (uri, method, body, headers) {
    ...
    connparams = {"url": uri, "method": method, "headers": headers};
